...
# CONFIGURATION
# -----------------------------
BASE_SECTION_URL = "https://secretnyc.co/escapes/page/{}/"
SCRIPT_DIR = os.path.dirname(__file__)
ROOT_DIR = os.path.dirname(SCRIPT_DIR)
OUTPUT_PATH = os.path.join(SCRIPT_DIR, "mementos_escapes_all_pages.json")
...
TOTAL_KNOWN_PAGES = 34  # Total pages from the Escapes section
...
def get_total_pages() -> int:
    ...
    try:
        ...
        logging.warning("Could not detect pages, using known total of 34")
        return TOTAL_KNOWN_PAGES
    except Exception as e:
        logging.error(f"Error getting total pages: {e}")
        logging.warning("Using known total of 34 pages")
        return TOTAL_KNOWN_PAGES
...
def scrape_article(article_url: str) -> Optional[Dict]:
    ...
    try:
        ...
        # ✅ Indoor filter - Modified for escapes to focus on travel destinations
        indoor_keywords = [
            # Transportation Hubs (allowed for escapes)
            # "airport", "terminal", "station", "port", "dock",

            # Dining and Nightlife (non-destination)
            ...
            # Sports and Recreation (indoor only)
            "gym", "fitness center", "studio", "spa", "salon", "wellness center",
            "bowling alley", "pool hall", "billiards", "indoor pool", "sports complex",
            "recreation center", "indoor court", "indoor track",

            # Entertainment (non-destination)
            ...
            # Generic Indoor Terms (unless part of a destination)
            "interior", "indoors", "climate controlled"
        ]

        # Add location-specific exceptions for indoor venues that are destinations
        destination_exceptions = [
            "castle", "mansion", "palace", "museum", "gallery", "theater",
            "cathedral", "church", "temple", "monastery", "historic", "heritage",
            "landmark", "monument", "ruins", "archaeological", "observatory",
            "aquarium", "zoo", "botanical garden", "conservatory", "greenhouse"
        ]

        # Check both title and description for indoor keywords
        combined_text = (title + " " + cleaned).lower()
        
        # First check if it's a destination exception
        is_destination = any(word in combined_text for word in destination_exceptions)
        
        # If it's not a destination, then check if it's an indoor venue to filter
        if not is_destination and any(word in combined_text for word in indoor_keywords):
            return None
