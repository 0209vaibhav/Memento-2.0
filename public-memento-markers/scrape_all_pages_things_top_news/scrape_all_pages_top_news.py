...
# CONFIGURATION
# -----------------------------
BASE_SECTION_URL = "https://secretnyc.co/top-news/page/{}/"
SCRIPT_DIR = os.path.dirname(__file__)
ROOT_DIR = os.path.dirname(SCRIPT_DIR)
OUTPUT_PATH = os.path.join(SCRIPT_DIR, "mementos_top_news_all_pages.json")
...
TOTAL_KNOWN_PAGES = 369  # Total pages from the Top News section
...
def get_total_pages() -> int:
    ...
    try:
        ...
        logging.warning("Could not detect pages, using known total of 369")
        return TOTAL_KNOWN_PAGES
    except Exception as e:
        logging.error(f"Error getting total pages: {e}")
        logging.warning("Using known total of 369 pages")
        return TOTAL_KNOWN_PAGES
...
def scrape_article(article_url: str) -> Optional[Dict]:
    ...
    try:
        ...
        indoor_keywords = [
            ...
            "theater", "theatre", "cinema", "movie", "screening", "gallery", "museum", "exhibit",
            "exhibition", "showcase", "auditorium", "concert hall", "venue", "stage", "performance",
            "show", "play", "musical", "opera",

            # Dining and Nightlife
            "restaurant", "cafe", "cafeteria", "bar", "pub", "club", "lounge", "dining room",
            "eatery", "bistro", "diner", "food hall", "brewery", "winery", "distillery",
            "speakeasy", "nightclub", "cocktail bar",

            # Shopping and Retail
            "mall", "shopping center", "store", "shop", "boutique", "market", "retail",
            ...
            "library", "classroom", "lecture hall", "workshop space", "conference room",
            "seminar", "convention center", "meeting room", "aquarium", "planetarium",

            # Entertainment Centers
            "arcade", "game center", "escape room", "vr", "virtual reality",
            "indoor playground", "trampoline park", "laser tag", "indoor park",
            "entertainment complex", "fun center",

            # Descriptive Terms
            "indoor", "inside", "interior", "indoors", "covered", "enclosed",
            "under roof", "climate controlled", "heated", "air conditioned"
        ]
        
        # Check both title and description for indoor keywords
        combined_text = (title + " " + cleaned).lower()
        if any(word in combined_text for word in indoor_keywords):
            return None
